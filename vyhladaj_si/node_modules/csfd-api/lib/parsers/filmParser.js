...
function parseSidebarList($, elements){
  const objects = []
  for(let i=0;i<elements.length;i++){
    var name = $(elements[i]).text()
    var id = parseInt($(elements[i]).attr('href').substr(6).split('-')[0])
...
function parseSimilar($){
  const similarTags = $('#sidebar .similar ul li a')
  return parseSidebarList($, similarTags)
}

function parseRelated($){
  const relatedTags = $('#sidebar .related ul li a')
  return parseSidebarList($, relatedTags)
}
